 ...
 *
 * A clean starting point for building an LVGL application on the P486 Panel
 * (720x720 MIPI DSI, capacitive touch).  Replace the body of ui_init() with
 * your own widgets.
 *
 * This stub doubles as a bring-up test:
 *   - "Hello, world!" follows your finger, centered on the touch point, so you
 *     can confirm touch coordinates line up with the display.  This is also the
 *     quickest way to check a rotated build: uncomment display.setRotation(90)
 *     in the sketch and the label should still track your finger exactly.
 *   - A rectangle cycles RED -> GREEN -> BLUE once per second so you can verify
 *     the panel's color order and timing are correct.
 *
 * Nothing here is pinned to a resolution -- every size is derived from the live
 * display size, so this same file works at 720x720 here and at 480x480 in the
 * sibling chipguy_P486Panel_480display library.
 *
...
void ui_init(void) {
    ...
    // Everything below is sized from the live resolution rather than fixed
    // pixels, so the layout holds at any panel size or rotation.
    ...
    // Color-test rectangle near the top.  The 3/4 x 7/24 proportions reproduce
    // the original 360x140 on a 480x480 panel and scale up from there.
    color_rect = lv_obj_create(screen);
    lv_obj_set_size(color_rect, (scr_w * 3) / 4, (scr_h * 7) / 24);
    lv_obj_align(color_rect, LV_ALIGN_TOP_MID, 0, scr_h / 12);
